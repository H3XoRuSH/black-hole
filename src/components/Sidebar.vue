<template>
  ...
  <div
    ...
    v-if="showSidebar"
  >
    <span class="font-bold text-gray-800 tracking-wide text-sm"
      >Gab's Arcade</span
    >
    <button
      @click="isOpen = !isOpen"
      class="p-1 focus:outline-none text-gray-600 hover:text-gray-900"
    >
      <svg
  ...
  >
    <div class="px-3 py-4">
      ...
      <div class="hidden md:block px-3 mb-6">
        <span class="font-bold text-lg text-gray-800 tracking-wide"
          >Gab's Arcade</span
        >
      </div>
    ...
    <div class="p-4 border-t border-gray-200 bg-gray-100/50">
      <div
        v-if="roomKey"
        class="px-3 py-2 rounded bg-white border border-gray-200 text-xs text-gray-600"
      >
        <span
          class="block text-gray-400 text-[10px] uppercase font-bold tracking-wide mb-0.5"
          >Active Room</span
        >
        <span class="font-mono font-semibold text-gray-700 tracking-wider">{{
          roomKey
        }}</span>
      </div>
